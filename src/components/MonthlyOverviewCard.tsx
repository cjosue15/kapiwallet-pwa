    ...
    <div className='bg-brand-card rounded-[24px] p-6 mb-8'>
      <div className='flex justify-between items-start mb-12'>
        <div>
          ...
          <p className='text-base font-light text-brand-text'>{monthLabel}</p>
        </div>
        <button
          onClick={onRefresh}
          disabled={loading}
        ...
        >
          <IconSymbol name='refresh-outline' size={24} color='#FFFEFF' />
        </button>
      </div>
      ...
      {error && <p className='mt-4 text-[13px] text-brand-expense text-center'>{error}</p>}
    </div>
  );
}
