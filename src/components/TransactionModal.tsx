...
export function TransactionModal({ visible, onClose, onSave, mode = 'create', initialValues }: TransactionModalProps) {
      ...
      <div className='relative w-full max-w-md bg-brand-background rounded-t-[24px] p-6 pb-10 max-h-[90vh] overflow-y-auto'>
          ...
          <div className='flex bg-[#2A2B2B] rounded-[30px] p-1 w-[85%]'>
            <button
              ...
              className={`flex-1 py-3 rounded-[26px] text-sm font-bold tracking-wide transition-colors ${
                transactionType === 'income' ? 'bg-brand-primary text-brand-text-primary' : 'text-[#666666]'
              }`}
            >
              INCOME
            </button>
            <button
              ...
              className={`flex-1 py-3 rounded-[26px] text-sm font-bold tracking-wide transition-colors ${
                transactionType === 'expense' ? 'bg-brand-primary text-brand-text-primary' : 'text-[#666666]'
              }`}
            >
              EXPENSE
            </button>
        ...
        <div className='flex flex-col items-center mb-12'>
          <div className='flex items-center justify-center mb-2 w-full'>
            <span
              className='text-[40px] font-bold text-brand-text mr-1 font-mono'
            ...
            <input
              ...
              className='flex-1 text-[60px] font-bold text-brand-text bg-transparent border-none outline-none w-40 text-center p-0 placeholder:text-center [-moz-appearance:textfield] font-mono'
              placeholder='0'
              step='0.01'
              style={{ fontFamily: "'JetBrains Mono', monospace" }}
            />
        ...
        >
          <span className='text-lg font-bold text-brand-text-primary'>{primaryActionLabel}</span>
        </button>

        {categoryPickerVisible && (
          <div className='fixed inset-0 z-[60] flex items-end justify-center bg-black/70'>
            <div className='absolute inset-0' onClick={closeCategoryPicker} />
                ...
                <div className='space-y-0'>
                  {categoryOptions.map((option) => (
                    ...
                    >
                      ...
                      <span className='flex-1 text-base font-semibold text-brand-text text-left'>{option.name}</span>
                      {selectedCategoryId === option.id && <IconSymbol name='checkmark' size={20} color='#B4DE00' />}
                    </button>
                  ))}
                </div>
              )}
